...
function main()
    ...
    # x2 = solution to bfgs relaxed problem
    x2 = Array{Array{Float64}}(undef, m)
    ...
    # lower bound from bfgs relaxed solution
    l_exact2::Array{Float64} = zeros(m)
    ...
    # computation time for bfgs method
    t2::Array{Float64} = zeros(m)
    ...
    for i::Int = i_min : m
      ...
      prob2 = createProblem(nd, x_L, x_U, 1, g_L, g_U, nd, Int64(nd * (nd + 1) / 2), eval_f, eval_g, eval_grad_f, eval_jac_g)
      addOption(prob2, "hessian_approximation", "limited-memory")
      addOption(prob2, "limited_memory_update_type", "bfgs")
      addOption(prob2, "tol", 1e-6)
      addOption(prob2, "max_iter", 100)
      prob2.x = r * ones(nd)
      val2, t2[i] = @timed status = solveProblem(prob2)
      x2[i] = prob2.x
      l_exact2[i] = prob.obj_val
      println("diff in solution", norm(x[i] - x2[i]))
    end
    ...
    #     coeff *= 2
    # end
    return x, x2, y, z, maxVio, t_sqp, t, t2, u, l_exact, l_exact2, l_approx, dval
end
